...
const ListRecommendCourse=(props)=>{
    ...
    useEffect(()=>{
        ...
        if(DATA.length===0)
        {
            const getRecommend=async()=>{
                const getUserInfo=await UserApi.getUser(authentication);
                const user=getUserInfo.payload;
                const getCourses=await UserApi.getRecommendCourse(user.id);
                console.log("Check recommend courses:");
                console.log(getCourses);
                setDATA(getCourses.payload);
                setIsLoading(false);
            }
            getRecommend();
...
export default ListRecommendCourse;